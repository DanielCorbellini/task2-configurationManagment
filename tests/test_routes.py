...
def test_login_post_success(client, mocker):
    """Test if the login post request works correctly."""
...
def test_login_post_failure(client, mocker):
    """Test if the login post request fails correctly."""
...
def test_logout_clears_session(client):
    """Test if the logout request clears the session correctly."""
    with client.session_transaction() as sess:
        sess["user_id"] = 1
...
def test_firewall_blocks_unauthenticated(client):
    """Test if the firewall blocks unauthenticated requests."""
    response = client.get("/lancamento")

    assert response.status_code == 302
    assert (
        "/login" in response.headers.get("Location", "") or b"/login" in response.data
    )


def test_firewall_allows_static(client):
    """Test if the firewall allows static requests."""
    response = client.get("/static/style.css")
    assert response.status_code != 302


def test_lancamento_page_renders(client, mocker):
    """Test if the lancamento page renders correctly."""
    with client.session_transaction() as sess:
        sess["user_id"] = 1
...
def test_lancamento_page_filters(client, mocker):
    """Test if the lancamento page filters correctly."""
    with client.session_transaction() as sess:
        sess["user_id"] = 1
    ...
    mock_list.assert_called_once_with(
        id_usuario=1, data_filtro="2024-01-01", situacao_filtro="PENDENTE"
    )


def test_lancamento_post_creates_and_redirects(client, mocker):
    """Test if the lancamento post request creates and redirects correctly."""
    with client.session_transaction() as sess:
        sess["user_id"] = 1
...
def test_exportar_pdf_renders(client, mocker):
    """Test if the exportar_pdf request renders correctly."""
    with client.session_transaction() as sess:
        sess["user_id"] = 1
    mocker.patch("app.listar_lancamentos", return_value=[])
...
def test_editar_lancamento_route_modal(client, mocker):
    """Test if the editar_lancamento route modal renders correctly."""
    with client.session_transaction() as sess:
        sess["user_id"] = 1
    mocker.patch(
        "app.buscar_lancamento_por_id", return_value={"id": 1, "descricao": "EDIT_ME"}
    )
    mocker.patch("app.listar_lancamentos", return_value=[])
    mocker.patch("app.listar_usuarios", return_value=[])
...
def test_editar_lancamento_post_saves(client, mocker):
    """Test if the editar_lancamento post request saves correctly."""
    with client.session_transaction() as sess:
        sess["user_id"] = 1
...
def test_deletar_lancamento_redirects(client, mocker):
    """Test if the deletar_lancamento route redirects correctly."""
    with client.session_transaction() as sess:
        sess["user_id"] = 1
